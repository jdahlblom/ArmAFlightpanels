...
namespace HidLibrary
{
    public class HidDevices
    {
        ...
        {
            var bufferSize = 0;
            var interfaceDetail = new NativeMethods.SpDeviceInterfaceDetailData { Size = IntPtr.Size == 4 ? 4 + Marshal.SystemDefaultCharSize : 8 };
        ...
        private static string GetDeviceInstanceId(IntPtr deviceInfoSet)
        {
            ...
            var ptrInstanceBuf = Marshal.AllocHGlobal(bufferSize);
            NativeMethods.CM_Get_Device_ID(deviceInfoSet, ptrInstanceBuf, bufferSize, 0);
            var instanceId = Marshal.PtrToStringAuto(ptrInstanceBuf);
