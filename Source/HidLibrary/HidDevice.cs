...
namespace HidLibrary
{
    public class HidDevice : IHidDevice
    {
        ...
        internal HidDevice(string devicePath, string description = null)
        {
            ...
            try
            {
                var hidHandle = OpenDeviceIO(_devicePath, NativeMethods.AccessNone);
        ...
        public void OpenDevice(DeviceMode readMode, DeviceMode writeMode, ShareMode shareMode)
        {
            ...
            try
            {
                Handle = OpenDeviceIO(_devicePath, readMode, NativeMethods.GenericRead | NativeMethods.GenericWrite, shareMode);
            }
            catch (Exception exception)
            {
                IsOpen = false;
            ...
            IsOpen = Handle.ToInt32() != NativeMethods.InvalidHandleValue;
        }
        ...
        public bool ReadFeatureData(out byte[] data, byte reportId = 0)
        {
            ...
            try
            {
                if (IsOpen)
                    ...
                    hidHandle = OpenDeviceIO(_devicePath, NativeMethods.AccessNone);
        ...
        public bool WriteFeatureData(byte[] data)
        {
            ...
            {
                Console.WriteLine(string.Format("Error accessing HID device '{0}'.", _devicePath), exception);
                throw new Exception(string.Format("Error accessing HID device '{0}'.", _devicePath), exception);
            }
            finally
            {
        ...
        private static IntPtr OpenDeviceIO(string devicePath, DeviceMode deviceMode, uint deviceAccess, ShareMode shareMode)
        {
            ...
            if (deviceMode == DeviceMode.Overlapped) flags = NativeMethods.FileFlagOverlapped;
